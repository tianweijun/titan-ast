...
size_t  BacktrackingBottomUpHash::operator()(const BacktrackingBottomUpBranch *t) const {
  return t->hashCode();
}

bool BacktrackingBottomUpEqual::operator()(const BacktrackingBottomUpBranch *t1, const BacktrackingBottomUpBranch *t2) const {
  return t1->equals(t2);
}
...
BacktrackingBottomUpAstAutomata::BacktrackingBottomUpAstAutomata(
    ...
    : tokenReducingSymbolInputStream(TokenReducingSymbolInputStream(innerGrammars, countOfInnerGrammars)),
      bottomUpBranchs(std::list<BacktrackingBottomUpBranch *>()),
      bottomUpBranchsShadow(std::unordered_set<BacktrackingBottomUpBranch *, BacktrackingBottomUpHash,BacktrackingBottomUpEqual>()),
      triedBottomUpBranchs(std::unordered_set<BacktrackingBottomUpBranch *, BacktrackingBottomUpHash,BacktrackingBottomUpEqual>()),
      astDfa(astDfa),
...
const std::list<Ast *> *BacktrackingBottomUpAstAutomata::buildAsts(std::list<Token *> *sourceTokens) {
  init(sourceTokens);
...
void BacktrackingBottomUpAstAutomata::doReduce(BacktrackingBottomUpBranch *bottomUpBranch, ProductionRule *closingProductionRule) {
  ...
  // 非空归约
  const SyntaxDfaState *reducingProductionRuleDfaState = closingProductionRule->reducingDfa->start;
  ...
  while (reducingSymbolListIt != bottomUpBranch->reducingSymbols.rend()) {
    ...
    auto nextReducingProductionRuleDfaStateIt = reducingProductionRuleDfaState->edges.find(inputReducingSymbol->reducedGrammar);
    if (nextReducingProductionRuleDfaStateIt != reducingProductionRuleDfaState->edges.end()) {
      nextReducingProductionRuleDfaState = nextReducingProductionRuleDfaStateIt->second;
    }
...
ReducingSymbol *BacktrackingBottomUpAstAutomata::getConnectedSignOfStartGrammarReducingSymbol() {
  ...
  connectedSignOfStartGrammarReducingSymbol->astOfCurrentDfaState = new AutomataTmpAst(startGrammar,nullptr);
...
bool BacktrackingBottomUpAstAutomata::addNewBacktrackingBottomUpBranch(BacktrackingBottomUpBranch *newBacktrackingBottomUpBranch) {
  auto findTriedBottomUpBranchsIt = triedBottomUpBranchs.find(newBacktrackingBottomUpBranch);
  if (findTriedBottomUpBranchsIt != triedBottomUpBranchs.end()) {
    return false;
  }

  auto findBottomUpBranchsShadowIt = triedBottomUpBranchs.find(newBacktrackingBottomUpBranch);
